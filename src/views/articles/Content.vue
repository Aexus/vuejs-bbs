  ...
  <div class="col-md-9 left-col pull-right">
    ...
    <div id="reply-box" class="reply-box form box-block">
      ...
      <div class="form-group reply-post-submit">
        <button id="reply-btn" :disabled="!auth" @click="comment" class="btn btn-primary">{{ commentId ? '保存编辑' : '回复'}}</button>
      ...
      if (this.auth) {
        ...
        simplemde.codemirror.on('keyup', (codemirror, event) => {
          if (event.ctrlKey && event.keyCode === 13) {
            this.comment()
          } else if (this.commentId && event.keyCode === 27) {  // 存在 commentId，且按下 Esc 键时,取消评论
